...
def subsample(X, y, yid, src_img, cell_idx, max_per_class: int, max_total: int, seed: int):
    ...
    y = y[keep_idx]
    yid = yid[keep_idx]
    src_img = src_img[keep_idx]
    cell_idx = cell_idx[keep_idx]
...
def choose_cmap(n_classes: int):
    if n_classes <= 10:
        return plt.cm.get_cmap("tab10", n_classes)
    if n_classes <= 20:
        return plt.cm.get_cmap("tab20", n_classes)
...
def main():
    ...
    if pd is not None:
        ...
        # Attach features (can be big). Comment these lines if undesired.
